 ...
 * 外部对话处理相关类型和接口
 */
export type ExternalChatAction = "SUMMARIZE" | "TAKEOVER";
...
export interface ExternalChatRequest {
  ...
  /** 操作类型：SUMMARIZE 整理对话 / TAKEOVER 接替任务 */
...
export interface FetchExternalChatResponse {
  /** 来源平台，如 deepseek */
  source: string;
  /** 对话标题 */
  title?: string;
...
/**
 * 从分享链接抓取对话内容（当前支持 DeepSeek）
 */
export async function fetchExternalChat(
  url: string,
): Promise<FetchExternalChatResponse> {
  return post<FetchExternalChatResponse>("/external-chat/fetch", { url });
}
...
export async function processExternalChat(
  ...
  return post<ExternalChatResponse>("/external-chat/process", request);
}
