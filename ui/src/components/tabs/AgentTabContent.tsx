...
import { formatDateTime, getAgentUsagi } from "../../utils";
...
const AgentTabContent: React.FC<AgentTabContentProps> = ({
  ...
  // 为每个 agent 分配 Usagi 头像（按 id 确定性分配）
  const agentsWithEmoji = useMemo(() => {
    return agents.map((agent) => ({
      ...agent,
      emoji: getAgentUsagi(agent.id),
    }));
  }, [agents]);
                ...
                >
                  <div className="flex items-start gap-3">
                    <img
                      src={agent.emoji}
                      alt={agent.name}
                      className="w-8 h-8 rounded-lg object-cover bg-gradient-to-br from-yellow-200 to-orange-200 shrink-0 mt-0.5"
                    />
