            ...
            {selectedKey === "model" && (
              <div>
                <div className="mb-4">
                  ...
                  <Select
                    options={(() => {
                      const opts: { value: ModelType; label: string }[] = [
                        {
                          value: "deepseek-chat",
                          label: "deepseek",
                        },
                      ];
                      // 兼容旧数据：若当前模型不在可选列表（如 glm-4.6），保留显示
                      if (
                        formData.model &&
                        !opts.some((o) => o.value === formData.model)
                      ) {
                        opts.push({ value: formData.model, label: formData.model });
                      }
                      return opts;
                    })()}
                    placeholder="请选择模型"
